...
func TestAnticycle_OnlyAffected(t *testing.T) {
	packages := []*model.Pkg{
		{
			Name: "bar",
...
func TestAnticycle_WithoutRelated(t *testing.T) {
	packages := []*model.Pkg{
		{
			Name: "bar",
			Path: "/tmp/anticycle/skipNotAffected/bar",
		...
		},
		{
			...
			Cycles: []*model.Cycle{
				{
					AffectedFile: "/tmp/anticycle/skipNotAffected/foo/foo.go",
					AffectedImport: &model.ImportInfo{
						Name:      "/tmp/anticycle/skipNotAffected/bar",
						NameShort: "bar",
						Alias:     nil,
	...
	result := withoutRelated(packages)
	assert.Len(t, result, 2)
...
func BenchmarkTestAnticycle_OnlyAffected(b *testing.B) {
	packages := []*model.Pkg{
		{
			Name: "bar",
			Path: "/tmp/anticycle/skipNotAffected/bar",
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		onlyAffected(packages)
	}
}

func BenchmarkTestAnticycle_WithoutRelated(b *testing.B) {
	packages := []*model.Pkg{
		{
			Name: "bar",
			Path: "/tmp/anticycle/skipNotAffected/bar",
